...
bool Pickable::PickingTrianglesCheck(baseObject * Target, D3DXVECTOR3 * pickPos, MouseRay ray,int* index)
{
	float u, v, t;
	...
	if (trans->GetnVerCnt() == 0) {
		...
		for (int i = 0; i < cnt; i++) {
			int a = ((D3DINDEX*)trans->GetpInData())[i]._0;
			int b = ((D3DINDEX*)trans->GetpInData())[i]._1;
			int c = ((D3DINDEX*)trans->GetpInData())[i]._2;
			if (D3DXIntersectTri(&pData[a].vPos, &pData[b].vPos, &pData[c].vPos, &ray.m_vPos, &ray.m_vDir, &u, &v, &t))
			{
				*pickPos = ray.m_vPos + (t*ray.m_vDir);
				if (index)
					*index = i;
